...
%build
USE_PGXS=1 PATH=%{pginstdir}/bin/:$PATH %{__make} %{__make} %{?_smp_mflags} %{with_llvm_arg}

%install
%{__rm} -rf %{buildroot}
USE_PGXS=1 PATH=%{pginstdir}/bin/:$PATH %{__make} %{__make} %{?_smp_mflags} %{with_llvm_arg} DESTDIR=%{buildroot} install
...
%changelog
* Sun Aug 30 2026 Devrim Gunduz <devrim@gunduz.org> - 1.4.1-3PGDG
